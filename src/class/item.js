...
class Item {
  static create(state, layerID, type, x, y, width, height, rotation) {
    ...
    let item = state.catalog.factoryElement(type, {
      ...
      name:
        ...
        "||" +
        state.catalog.getIn(["elements", type, "info", "price"]) +
        "||" +
        state.catalog.getIn(["elements", type, "info", "price1"]) +
        "||" +
        state.catalog.getIn(["elements", type, "info", "price2"]),
      type,
      height,
      width,
      x,
  ...
  //imantado
  static snapToGrid(state, layerID, itemID) {
    const gridSizeX = 5;
    const gridSizeY = 20;
    const item = state.getIn(["scene", "layers", layerID, "items", itemID]);
    const snapX = Math.round(item.x / gridSizeX) * gridSizeX;
    const snapY = Math.round(item.y / gridSizeY) * gridSizeY;
    const snappedItem = item.merge({
      x: snapX,
      y: snapY,
    });
